...
| [14_biophysical_features](14_biophysical_features/) | Do biology-informed **features** help? Three additions to the readout on a frozen trunk — contact-weighted pooling, burial+biophysics, MSA conservation — each cluster-bootstrapped on **two** blind corpora (S669 62 prot., FireProt ≤500 138 prot.). | **Only the pooling change works.** Weighting the pair-track pool by Boltz's own predicted contacts gains FireProt r **+0.067 [+0.006, +0.126]**, ρ **+0.079**, MAE **−0.139** at *identical* dims; explicit biophysics and conservation add **nothing** (redundant with the trunk). It is a **domain-shift defence**, not an information gain — in-distribution it is marginally worse. |
