...
|---|---|---|
| **`01_heatmap_mean.png`** | Predicted ΔΔG for every substitution at each scanned position (mean of regimes A/B/D). Rows = mutant residue grouped by chemistry; columns = the scanned sites. | A data view — **not in report.pdf**. Grey = no value (wild-type cell, or not computed), and at 33% of the mutable grid it dominates the image; 54 of the 177 columns have under 10 of 19 substitutions scored, so per-column reads are unsafe. §3.1 of the report states the landscape structure numerically instead. Columns are **not contiguous in sequence** — 177 selected sites. |
