...
## Reproducing their Figure 1, and what it added

Correlation shows a predictor tracks the label; it does not show that it participates in the
**mechanism** the original work described. Their Figure 1 places every variant on a
stability × conservation plane and scores each sector by how many of its variants lose
function. Re-running it with our ΔΔG in Rosetta's place asks a sharper question, and three
things came out of it.

**The reproduction is faithful.** With Rosetta's own ΔΔG on our 13-dataset subset, the two
corner sectors the paper quotes in its text come back at **84 % / 96 %** against their
published **81 % / 93 %**. `paper_figures.py` aborts if they drift past 15 points, so the
figures cannot be generated on a broken harness.

**Our ΔΔG reproduces the landscape on a compressed axis.** sd 0.97 kcal/mol against Rosetta's
2.14, so the paper's absolute cuts leave our top ΔΔG column nearly empty (46 variants against
1,129). At quantile-matched cuts the sector structure is near-identical — high-ΔΔE row
68/81/88/96 % against 71/81/88/96 %.

**About half the advantage over Rosetta is conservation** — a first bound on the open question
below, obtained with no GPU. AUC for detecting loss of function:

| | Rosetta | ours | Δ AUC | 95 % CI |
|---|---|---|---|---|
| pooled | 0.710 | 0.759 | **+0.048** | [+0.014, +0.079] ✔ |
| within ΔΔE strata | 0.633 | 0.654 | +0.021 | [−0.010, +0.052] ✗ |

Conditioning removes 57 % of the gap and what remains no longer clears zero. Read it in both
directions: the residual is positive in **all four** strata (+0.018 to +0.023), so this is the
power available from 11 proteins, not an established absence.

## The stability assay decides — a within-protein control

NUDT15 contributes two of the thirteen datasets: **VAMP-seq abundance** (cellular protein
level, the closest readout of stability in the corpus) and **drug sensitivity** (enzyme
function). Same sequence, same structure, same alignment, **the same ΔΔG predictions** — only
the measured quantity changes.

| \|ρ\| direct | abundance (VAMP-seq) | drug sensitivity |
|---|---|---|
| GEMME ΔΔE (conservation) | 0.333 | **0.557** |
| Rosetta ΔΔG | 0.531 | 0.270 |
| **our ΔΔG** | **0.675** | 0.438 |

**The ordering inverts.** On the function assay conservation wins; on the stability assay both
ΔΔG predictors overtake it and ours leads by **+0.144 [+0.076, +0.213]** over Rosetta and
**+0.340 [+0.247, +0.434]** over GEMME (cluster bootstrap over the protein's 156 positions —
it answers whether *this dataset's* gap is real, **not** whether it generalises). This is the
paper's central claim reproduced inside one protein, and the clearest evidence that what our
model extracts is stability rather than general variant tolerance.

It is also **partial counter-evidence to the conservation account above**: here conservation is
*weak* (0.333) and we score roughly double it, whereas a predictor whose edge were mostly
smuggled conservation should fail where conservation fails. The two results are jointly
consistent with a ΔΔG that is part evolutionary and part structural.

Note also that on this dataset adding conservation **hurts** our arm — ΔΔG-only 0.518 against
0.429 for ΔΔG+ΔΔE and 0.439 for the full 47-feature model. On the purest stability readout in
the corpus, our single-feature model beats every richer one.

## Figures

![paired LOPO](figures/01_lopo_paired.png)

Five figures; see [`figures/README.md`](figures/README.md) for the index, the colour
provenance and the **ΔΔE orientation gotcha** (the deposited `gemme_dde` column runs opposite
to the paper's ΔΔE). The paper-facing write-up is [`report.pdf`](report.pdf), regenerated by
`build_report.py` from the committed tables so it cannot drift from them.
...
- **The MSA-confound test.** Rebuild this corpus with `no_msa: true` (results/04's
  ...
  **Now partly bounded and sharper, not superseded:** the strata analysis puts ~57 % of the
  pooled advantage on conservation, and the VAMP-seq dissociation shows it cannot be all of
  it. That gives the ablation a falsifiable prediction — if the gain were purely evolutionary
  the conditional Δ AUC should fall to ~0, and single-sequence ΔΔG should lose the VAMP-seq
  lead it currently holds by +0.144.
...
| Result tables | `layer1_direct.csv`, `layer2_lopo_summary.csv`, `layer2_lopo_per_dataset.csv`, `bootstrap_protein{,_noubi4}.csv`, `conservation_strata_auc.csv`, `vampseq_{dissociation,bootstrap}.csv` |
...
python results/15_mave_stability_transfer/paper_figures.py   # Fig 1 reproduction, strata, VAMP-seq
python results/15_mave_stability_transfer/build_report.py    # -> report.pdf
```
