...
class Boltz2(LightningModule):
    ...
    def __init__(
        ...
        # self.track_recycling = track_recycling
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            ...
            # Always collect recycling steps for potential write_embeddings  
            recycling_s_list = []  # list of (B, L, token_s)
            recycling_z_list = []  # list of (B, L, L, token_z)
            recycling_pdist_list = []
            ...
            if self.run_trunk_and_structure:
                for i in range(recycling_steps + 1):
                    with torch.set_grad_enabled(
                        ...
                        # Always track per-recycling embeddings
                        recycling_s_list.append(s.detach().clone())
                        recycling_z_list.append(z.detach().clone())
                        # Compute distogram for este paso intermedio (sin gradientes)
                        with torch.no_grad():
                            recycling_pdist_list.append(
                                self.distogram_module(z).detach().clone()
                            )
            ...
            # Always attach recycling traces stacked together
            # Stack along first dimension = number of recycling rounds + 1 (final)
            # Shapes: (R+1, B, L, token_s) and (R+1, B, L, L, token_z)
            # Note: final embeddings are already included from the last iteration of the loop
            dict_out["recycling_s"] = torch.stack(recycling_s_list, dim=0)
            dict_out["recycling_z"] = torch.stack(recycling_z_list, dim=0)
            dict_out["recycling_pdistogram"] = torch.stack(recycling_pdist_list, dim=0)
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> dict:
        try:
            ...
            # Include final pdistogram logits so writer can always save it when --write_embeddings
            if "pdistogram" in out:
                pred_dict["pdistogram"] = out["pdistogram"]

            # Optional recycling traces
            if "recycling_s" in out:
                pred_dict["recycling_s"] = out["recycling_s"]
            if "recycling_z" in out:
                pred_dict["recycling_z"] = out["recycling_z"]
            if "recycling_pdistogram" in out:
                pred_dict["recycling_pdistogram"] = out["recycling_pdistogram"]

            if "keys_dict_out" in self.predict_args:
                for key in self.predict_args["keys_dict_out"]:
                    pred_dict[key] = out[key]
            # sample_atom_coords is absent when skip_run_structure=True
            if "sample_atom_coords" in out:
                pred_dict["coords"] = out["sample_atom_coords"]
            else:
                # Keep key absent; writer checks for coords existence
                pred_dict["coords"] = None
            if self.confidence_prediction:
