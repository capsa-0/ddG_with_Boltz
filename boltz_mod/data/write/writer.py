...
class BoltzWriter(BasePredictionWriter):
    ...
    def _to_numpy(t: Tensor) -> np.ndarray:
        """Safely convert a tensor that may be in bfloat16/half to a float32 numpy array.

        NumPy does not yet universally support PyTorch bfloat16; converting to float32
        ensures compatibility when saving with np.savez_compressed.
        """
        if t is None:
            return None  # type: ignore
        if not isinstance(t, torch.Tensor):  # defensive
            raise TypeError("Expected a torch.Tensor")
        # Always move to CPU and cast to float32 for portability
    ...
    def write_on_batch_end(
        ...
        # Determine if structure coords are available (may be skipped with --skip_run_structure)
        has_coords = "coords" in prediction and prediction["coords"] is not None
        if has_coords:
        ...
        for record_idx, record in enumerate(records):
            ...
            # If we do not have coordinates (skip_run_structure flow), we still may want embeddings/recycling
            if not has_coords:
                # New layout: separate files per embedding type with all steps stacked
                if self.write_embeddings:
                    # s embedding: stack recycling steps + final
                    if "recycling_s" in prediction:
                        s_path = struct_dir / f"s.npz"
                        np.savez_compressed(s_path, s=self._to_numpy(prediction["recycling_s"]))
                        print(f"s embedding (all steps) saved in: {s_path}")  # noqa: T201
                    # z embedding: stack recycling steps + final  
                    if "recycling_z" in prediction:
                        z_path = struct_dir / f"z.npz"
                        np.savez_compressed(z_path, z=self._to_numpy(prediction["recycling_z"]))
                        print(f"z embedding (all steps) saved in: {z_path}")  # noqa: T201
                    # pdistogram: stack recycling steps + final
                    if "recycling_pdistogram" in prediction:
                        p_path = struct_dir / f"pdistogram.npz"
                        np.savez_compressed(p_path, pdistogram=self._to_numpy(prediction["recycling_pdistogram"]))
                        print(f"pdistogram (all steps) saved in: {p_path}")  # noqa: T201
                # Continue to next record (no structure writing)
                continue

            coord = coords[record_idx]
            ...
            for model_idx in range(coord.shape[0]):
                ...
                # Save the structure
                # struct_dir already created above
            ...
            # Save embeddings (final) + recycling traces
            # New layout when structure path is active (after coords writing) 
            if self.write_embeddings:
                # s embedding: stack recycling steps + final
                if "recycling_s" in prediction:
                    s_path = struct_dir / f"s_{record.id}.npz"
                    np.savez_compressed(s_path, s=self._to_numpy(prediction["recycling_s"]))
                    print(f"s embedding (all steps) saved in: {s_path}")  # noqa: T201
                # z embedding: stack recycling steps + final
                if "recycling_z" in prediction:
                    z_path = struct_dir / f"z_{record.id}.npz"
                    np.savez_compressed(z_path, z=self._to_numpy(prediction["recycling_z"]))
                    print(f"z embedding (all steps) saved in: {z_path}")  # noqa: T201
                # pdistogram: stack recycling steps + final
                if "recycling_pdistogram" in prediction:
                    p_path = struct_dir / f"pdistogram_{record.id}.npz"
                    np.savez_compressed(p_path, pdistogram=self._to_numpy(prediction["recycling_pdistogram"]))
                    print(f"pdistogram (all steps) saved in: {p_path}")  # noqa: T201
