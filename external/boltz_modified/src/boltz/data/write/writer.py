...
class BoltzWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(
        ...
        # Check if coordinates are available (skipped in embeddings_only mode)
        if "coords" not in prediction:
            # Only write embeddings, skip structure writing
            struct_dir = self.output_dir / records[0].id if records else self.output_dir
            ...
            if self.write_embeddings and "s" in prediction and "z" in prediction:
                ...
                # Add pdistogram if available
                if "pdistogram" in prediction:
                    embeddings_dict["pdistogram"] = prediction["pdistogram"].cpu().numpy()

                path = struct_dir / f"embeddings_{records[0].id}.npz"
